  ...
  return (
    <div className='flex items-center justify-center flex-col pt-20 bg-[#f2fff8]'>
      ...
      <div className='flex items-center gap-10 '>
        {plans.map((item)=>{
          return(
            <div className='bg-white border-[1px] border-[#EDEDED] p-8 rounded-[10px] shadow-lg'>
              <img  className='w-[31px] h-[31px] ' src={assets.logo_icon} alt="" />
              <p className='text-[#515151] text-[20px] font-[500]'>{item.id}</p>
              <p className='text-[#515151] text-[15px] font-[400]'>{item.desc}</p>
              <p>{item.desc}</p>
              <div>
                <p>${item.price}</p>
